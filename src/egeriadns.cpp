...
EgeriaDns::EgeriaDns(const char *bind_ip, uint16_t port_no,
    ...
    memset(this, 0, sizeof(EgeriaDns)); // Clear previous state
    m_verbose = verbose;

    // Create and socket
    int ret = socket(PF_INET, SOCK_DGRAM, 0);
    if(ret < 0) {
      throw runtime_error("EgeriaDns::EgeriaDns: Cannot create socket");
    } else {
      m_sockfd = ret;
    }
    ...
    if(bind(m_sockfd, (struct sockaddr *) &m_address,
      ...
      char buf[80];
      sprintf(buf, "EgeriaDns::EgeriaDns: Cannot bind to port %u", port_no);
    ...
    m_dap_ht  = (allowed_len | m_gw_suf_len)? (DNSAP*)calloc(EGERIADNS_DAPSIZE, sizeof(DNSAP)) : NULL;
    ...
    if(m_value == NULL) 
      throw runtime_error("EgeriaDns::EgeriaDns: Cannot allocate buffer");
...
void EgeriaDns::StatRun(void *p) {
  EgeriaDns *obj = (EgeriaDns*)p;
  obj->Run();
...
void EgeriaDns::HandlePacket() {
  ...
  if(m_verbose > 3) {
    ...
    printf("\tEgeriaDns::HandlePacket: ANCount: %d\n", m_hdr->ANCount);
    printf("\tEgeriaDns::HandlePacket: NSCount: %d\n", m_hdr->NSCount);
    printf("\tEgeriaDns::HandlePacket: ARCount: %d\n", m_hdr->ARCount);
  }
...
//*  uint16_t zCount = m_hdr->ANCount | m_hdr->NSCount | m_hdr->ARCount | (m_hdr->Bits & (m_hdr->QR_MASK | m_hdr->TC_MASK));
  ...
  m_hdr->Transcode();
} // EgeriaDns::HandlePacket

/*---------------------------------------------------*/
uint16_t EgeriaDns::HandleQuery() {
  // Decode qname
  ...
  *--key_end = 0; // Remove last dot, set EOLN
  ...
  return 0;
} // EgeriaDns::HandleQuery

/*---------------------------------------------------*/
int EgeriaDns::TryMakeref(uint16_t label_ref) {
  char val2[VAL_SIZE];
  char *tokens[MAX_TOK];
  int ttlqty = Tokenize("TTL", NULL, tokens, strcpy(val2, m_value));
  m_ttl = ttlqty? atoi(tokens[0]) : 24 * 3600;
  ...
  m_hdr->ANCount = 0;
  printf("EgeriaDns::TryMakeref: Generated REF NS=%u\n", m_hdr->NSCount);
  return m_hdr->NSCount;
} //  EgeriaDns::TryMakeref
/*---------------------------------------------------*/

int EgeriaDns::Tokenize(const char *key, const char *sep2, char **tokens, char *buf) {
  int tokensN = 0;
...
void EgeriaDns::Answer_ALL(uint16_t qtype, char *buf) {
  const char *key;
  switch(qtype) {
      case  1 : key = "A";      break;
      case  2 : key = "NS";     break;
...
void EgeriaDns::Fill_RD_IP(char *ipddrtxt, int af) {
  uint16_t out_sz;
  switch(af) {
  ...
  Out4(inetaddr);
#endif
} // EgeriaDns::Fill_RD_IP

/*---------------------------------------------------*/

void EgeriaDns::Fill_RD_DName(char *txt, uint8_t mxsz, int8_t txtcor) {
  uint8_t *snd0 = m_snd;
...
int EgeriaDns::LocalSearch(const uint8_t *key, uint8_t pos, uint8_t step) {
  ...
  return 1;
} // EgeriaDns::LocalSearch


/*---------------------------------------------------*/
// Returns x>0 = hash index to update size; x<0 = disable;
DNSAP *EgeriaDns::CheckDAP(uint32_t ip_addr) {
  uint32_t hash = ip_addr * m_daprand;
  hash ^= hash >> 16;
  hash += hash >> 8;
  DNSAP *dap = m_dap_ht + (hash & (EGERIADNS_DAPSIZE - 1));
  ...
  return (dap->ed_size <= EGERIADNS_DAPTRESHOLD)? dap : NULL;
} // EgeriaDns::CheckDAP

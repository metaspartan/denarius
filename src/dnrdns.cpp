...
DnrDns::DnrDns(const char *bind_ip, uint16_t port_no,
    ...
    memset(this, 0, sizeof(DnrDns)); // Clear previous state
    m_verbose = verbose;

    // Create and socket
    int ret = socket(PF_INET, SOCK_DGRAM, 0);
    if(ret < 0) {
      throw runtime_error("DnrDns::DnrDns: Cannot create socket");
    } else {
      m_sockfd = ret;
    }
    ...
    if(bind(m_sockfd, (struct sockaddr *) &m_address,
      ...
      char buf[80];
      sprintf(buf, "DnrDns::DnrDns: Cannot bind to port %u", port_no);
    ...
    m_dap_ht  = (allowed_len | m_gw_suf_len)? (DNSAP*)calloc(DNRDNS_DAPSIZE, sizeof(DNSAP)) : NULL; 
    ...
    if(m_value == NULL) 
      throw runtime_error("DnrDns::DnrDns: Cannot allocate buffer");
	...
	if(c == '.') {
	  if(p[1] > 040) { // if allowed domain is not empty - save it into ht
	    step |= 1;
	    if(m_verbose > 3)
	      printf("\tDnrDns::DnrDns: Insert TLD=%s: pos=%u step=%u\n", p + 1, pos, step);
	    do 
	      pos += step;
	...
	step |= 1;
	if(m_verbose > 3)
	  printf("\tDnrDns::DnrDns: Insert Local:[%s]->[%s] pos=%u step=%u\n", p, p_eq, pos, step);
	do 
	  pos += step;
        while(m_ht_offset[pos] != 0);
    ...
    if(m_verbose > 0)
	 printf("DnrDns::DnrDns: Created/Attached: %s:%u; Qty=%u:%u\n", 
...
DnrDns::~DnrDns() {
    ...
    free(m_dap_ht);
    if(m_verbose > 0)
	 printf("DnrDns::~DnrDns: Destroyed OK\n");
} // DnrDns::~DnrDns


/*---------------------------------------------------*/

void DnrDns::StatRun(void *p) {
  DnrDns *obj = (DnrDns*)p;
  obj->Run();
...
/*---------------------------------------------------*/
void DnrDns::Run() {
  if(m_verbose > 2) printf("DnrDns::Run: started\n");
  ...
  if(m_verbose > 2) printf("DnrDns::Run: Received Exit packet_len=%d\n", m_rcvlen);

} //  DnrDns::Run

/*---------------------------------------------------*/

void DnrDns::HandlePacket() {
  if(m_verbose > 2) printf("DnrDns::HandlePacket: Handle packet_len=%d\n", m_rcvlen);
  ...
  if(m_verbose > 3) {
    printf("\tDnrDns::HandlePacket: msgID  : %d\n", m_hdr->msgID);
    printf("\tDnrDns::HandlePacket: Bits   : %04x\n", m_hdr->Bits);
    printf("\tDnrDns::HandlePacket: QDCount: %d\n", m_hdr->QDCount);
    printf("\tDnrDns::HandlePacket: ANCount: %d\n", m_hdr->ANCount);
    printf("\tDnrDns::HandlePacket: NSCount: %d\n", m_hdr->NSCount);
    printf("\tDnrDns::HandlePacket: ARCount: %d\n", m_hdr->ARCount);
  }
...
uint16_t DnrDns::HandleQuery() {
  ...
  *--key_end = 0; // Remove last dot, set EOLN
  ...
  if(m_verbose > 3) 
    printf("DnrDns::HandleQuery: After TLD-suffix cut: [%s]\n", key);
  ...
  if(p) {
    ...
    if(m_allowed_qty) { // Activated TLD-filter
      if(*p != '.') {
        if(m_verbose > 3) 
      printf("DnrDns::HandleQuery: TLD-suffix=[.%s] is not specified in given key=%s; return NXDOMAIN\n", p, key);
	return 3; // TLD-suffix is not specified, so NXDOMAIN
      ...
      do {
        ...
        if(m_ht_offset[pos] == 0) {
          if(m_verbose > 3) 
  	    printf("DnrDns::HandleQuery: TLD-suffix=[.%s] in given key=%s is not allowed; return NXDOMAIN\n", p, key);
...
/*---------------------------------------------------*/
int DnrDns::TryMakeref(uint16_t label_ref) {
  char val2[VAL_SIZE];
  char *tokens[MAX_TOK];
  int ttlqty = Tokenize("TTL", NULL, tokens, strcpy(val2, m_value));
  m_ttl = ttlqty? atoi(tokens[0]) : 24 * 3600;
  ...
  m_hdr->ANCount = 0;
  printf("DnrDns::TryMakeref: Generated REF NS=%u\n", m_hdr->NSCount);
  return m_hdr->NSCount;
} //  DnrDns::TryMakeref
/*---------------------------------------------------*/

int DnrDns::Tokenize(const char *key, const char *sep2, char **tokens, char *buf) {
  int tokensN = 0;
...
void DnrDns::Answer_ALL(uint16_t qtype, char *buf) {
  const char *key;
  switch(qtype) {
      case  1 : key = "A";      break;
      case  2 : key = "NS";     break;
  ...
  if(m_verbose > 0) printf("DnrDns::Answer_ALL(QT=%d, key=%s); TokenQty=%d\n", qtype, key, tokQty);
  ...
  for(int tok_no = 0; tok_no < tokQty; tok_no++) {
      if(m_verbose > 1) 
	printf("\tDnrDns::Answer_ALL: Token:%u=[%s]\n", tok_no, tokens[tok_no]);
      Out2(m_label_ref);
...
void DnrDns::Fill_RD_IP(char *ipddrtxt, int af) {
  uint16_t out_sz;
  switch(af) {
...
void DnrDns::Fill_RD_DName(char *txt, uint8_t mxsz, int8_t txtcor) {
  uint8_t *snd0 = m_snd;
...
} // DnrDns::Fill_RD_DName

/*---------------------------------------------------*/
/*---------------------------------------------------*/

int DnrDns::Search(uint8_t *key) {
  if(m_verbose > 1) 
    printf("DnrDns::Search(%s)\n", key);
...
int DnrDns::LocalSearch(const uint8_t *key, uint8_t pos, uint8_t step) {
  if(m_verbose > 1) 
    printf("DnrDns::LocalSearch(%s, %u, %u) called\n", key, pos, step);
    do {
      pos += step;
      if(m_ht_offset[pos] == 0) {
        if(m_verbose > 3) 
  	  printf("DnrDns::LocalSearch: Local key=[%s] not found; go to nameindex search\n", key);
...
DNSAP *DnrDns::CheckDAP(uint32_t ip_addr) { 
  uint32_t hash = ip_addr * m_daprand;
  hash ^= hash >> 16;
  hash += hash >> 8;
  DNSAP *dap = m_dap_ht + (hash & (DNRDNS_DAPSIZE - 1));
  ...
  return (dap->ed_size <= DNRDNS_DAPTRESHOLD)? dap : NULL;
} // DnrDns::CheckDAP 

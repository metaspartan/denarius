...
extern bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CTransaction& txTo, unsigned int nIn, unsigned int flags, bool fValidatePayToScriptHash, int nHashType);
...
// Generally:  GetNameOpFee() > IsNameFeeEnough().
int64_t GetNameOpFee(const CBlockIndex* pindexBlock, const int nRentalDays, int op, const vector<unsigned char> &vchName, const vector<unsigned char> &vchValue)
{
    ...
    const CBlockIndex* lastBlock = GetLastBlockIndex(pindexBlock, false);

    int64_t txMinFee = nRentalDays * lastBlock->nMint / (365 * 100); // 1% block per 365 days

    if (op == OP_NAME_NEW)
        txMinFee += lastBlock->nMint / 100; // +1% block per operation itself
...
bool SignNameSignature(const CTransaction& txFrom, CTransaction& txTo, unsigned int nIn, int nHashType=SIGHASH_ALL, CScript scriptPrereq=CScript())
{
    ...
    if (scriptPrereq.empty())
        if (!VerifyScript(txin.scriptSig, txout.scriptPubKey, txTo, nIn, STANDARD_SCRIPT_VERIFY_FLAGS, false, 0))
            return false;
        ...
        LOCK2(cs_main, pwalletMain->cs_wallet);
        {
            ...
            for (;;)
            {
                ...
                if (nTotalValue - nWtxinCredit > 0)
                    if (!pwalletMain->SelectCoins2(nTotalValue - nWtxinCredit, wtxNew.nTime, setCoins, nValueIn))
                    {
